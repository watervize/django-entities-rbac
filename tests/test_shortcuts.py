...
from entities_rbac.exceptions import RoleDoesNotExist, EntitiesRBACScopeException
...
class RemoveRoleTests(TestCase):
    ...
    def test_remove_role_from_user_with_multiple_roles(self):
        ...
        self.assertListEqual(
            [self.Anesthesiologist, self.Surgeon], get_user_roles(self.user)
        )
...
class GetUserRoleTests(TestCase):
    ...
    def test_ensure_user_roles_are_in_order(self):
        ...
        self.assertListEqual(
            [ShoRole3, ShoRole1, ShoRole2, ShoRole4], get_user_roles(user)
        )
    ...
    def test_queries_with_prefetch(self):
        ...
        fetched_user = (
            get_user_model().objects.prefetch_related("groups").get(pk=self.user.pk)
        )
        N = 3
...
class AvailablePermNamesTests(TestCase):
    def assert_available_perm_names_equals_available_perm_status(self):
        ...
        self.assertEqual(
            set(perm_names), set(p for p, has_perm in perm_hash.items() if has_perm)
        )
    ...
    def test_queries_no_prefetch(self):
        ...
        with self.assertNumQueries(
            2 * N
        ):  # Two query (fetch roles, fetch permissions) per call
            for i in range(N):
